# ---------------------------------------------------------------------------------------
# Installs OpenStack Kilo using PackStack: MANILA only
...
if [ -f provision-packstack-kilo.log ]; then
  echo '' > provision-packstack-kilo.log
fi
exec 1> >(tee -a provision-packstack-kilo.log) 2>&1

#fill these values in accordingly
if [ "$#" -ne 8 ]; then
  echo 'ERROR. usage: ./provision-packstack-kilo.sh <answer-file-name> <cluster-mgt-lif-ip> <svm-name> <cdot-username> <cdot-password> <external-eth-interface> <private-eth-interface> <storage-eth-interface>'
  exit 1
fi
answer_file_name="$1"
...
if [ -z "$(yum list installed | grep openstack-packstack | grep kilo)" ]; then
  sudo yum update -y
...
#stop openstack nova services. Is still up, packstack install won't complete
...
while [ $runthrice -lt 4 ]; do
  if  [ $status -ne 0 ] && ([ -n $(grep 'Could not prefetch glance_image provider' provision-packstack-kilo.log) ] || [ -n $(grep 'Error: cinder type-create iscsi returned 1 instead of one of [0]' provision-packstack-kilo.log) ]); then
...
if [ $runthrice -lt 4 ]; then
  ...
  sudo chown rdouser:centos /home/rdouser/keystonerc_admin 
  ...
  #start any nova and neutron services not started 
  svcs='openstack-nova-api,openstack-nova-cert,openstack-nova-consoleauth,openstack-nova-scheduler,openstack-nova-conductor,openstack-nova-novncproxy,neutron-server,neutron-dhcp-agent,neutron-l3-agent,neutron-metadata-agent'
  IFS=,
  for s in $svcs; do
    st=$(service "$s" status)
    if [[ ! $st == *"running"* ]]; then
